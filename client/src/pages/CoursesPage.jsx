...
export default function CoursesPage() {
  ...
  useEffect(() => {
    async function fetchCourses() {
      try {
        ...
        // Map enrolled courses and enforce 14-day Launch Test rule
  ...
  const handleLaunchCourse = async (course) => {
    ...
    try {
      ...
      console.error('Failed to generate launch test', err);
      // Fallback: Just let them in if AI fails
      window.location.href = `/dashboard/courses/${course.id}`;
    } finally {
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 w-full mb-20">
      <div className="mb-12 space-y-2">
        <h1 className="text-5xl font-black tracking-tighter text-white">
          Explore <span className="text-accent underline decoration-primary/30 underline-offset-8">Courses</span>
        </h1>
        <p className="text-white/40 font-medium tracking-widest uppercase text-xs">Curated Learning paths for you</p>
      </div>
        ...
        {courses.map(course => (
          <Card key={course.id} className="glass-card-premium neon-border-primary border-white/5 flex flex-col group h-full">
            <CardHeader className="flex-1 space-y-4">
              ...
              <CardDescription className="line-clamp-3 text-white/50 text-sm leading-relaxed font-medium">
                {course.description}
              </CardDescription>
            </CardHeader>
                  ...
                  {quizSubmitted && (
                    <div className={`text-center p-6 rounded-2xl border ${quizScore >= 60 ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
                      ...
                      <button 
                        ...
                        className="uiverse-btn mt-4 px-8"
                      >
                        Enter Course
                      </button>
                    </div>
                    ...
                    {launchTestData.map((q, qIdx) => (
                      <div key={qIdx} className="bg-white/[0.02] border border-white/5 rounded-2xl p-6 space-y-4">
                        <p className="font-bold text-white/90 text-sm">
                          <span className="text-primary mr-2">Q{qIdx + 1}.</span>{q.question}
                        </p>
