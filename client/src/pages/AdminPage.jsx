    ...
    <div className="animate-fade-in-up w-full mb-32 space-y-12">
      <div className="space-y-4">
        <h1 className="text-5xl font-black tracking-tighter text-black uppercase italic leading-none">
          Control <span className="text-gray-300">Phase</span>
        </h1>
        <p className="text-gray-400 text-[9px] font-black tracking-[0.4em] uppercase ml-1">Universal Platform Override / Administrative Matrix</p>
      </div>
      ...
      {error && (
        <div className="bg-gray-50 border border-black/10 text-black p-6 rounded-[2rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-4">
           <Shield className="text-black" size={16} /> ERROR: {typeof error === 'object' ? error.message : error}
        </div>
      )}
      {success && (
        <div className="bg-black text-white p-6 rounded-[2rem] text-[10px] font-black uppercase tracking-widest text-center shadow-xl shadow-black/10">
           {success}
        </div>
      )}
        ...
        <div className="space-y-12">
          ...
          <Card className="bg-white border border-gray-100 rounded-[3rem] p-10 shadow-sm relative overflow-hidden">
             ...
             <div className="flex items-center gap-4 mb-10 pb-6 border-b border-gray-50 relative z-10">
                <div className="w-10 h-10 rounded-xl bg-black flex items-center justify-center text-white"><Plus size={20} /></div>
                <h3 className="text-sm font-black text-black uppercase tracking-widest">Register New Domain</h3>
             </div>
             ...
             <form onSubmit={handleCreate} className="space-y-8 relative z-10">
                ...
                <div className="space-y-2">
                  ...
                  <input value={newPlaylistUrl} onChange={e => setNewPlaylistUrl(e.target.value)} placeholder="HTTPS://YOUTUBE.COM/..."
                    className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-5 py-4 text-xs font-bold text-black focus:border-black focus:outline-none transition-all" />
                </div>
                <button type="submit" disabled={creating} className="uiverse-btn !rounded-xl !py-4 shadow-xl shadow-black/5 flex items-center justify-center gap-4 w-full md:w-auto md:px-12">
          ...
          <div className="space-y-8">
             <div className="space-y-1">
                <h3 className="text-3xl font-black italic tracking-tighter text-black uppercase">Active Nodes</h3>
                <p className="text-[9px] font-black tracking-[0.3em] text-gray-400 uppercase">SYNCHRONIZED DOMAINS: {courses.length}</p>
             </div>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {loading ? <div className="col-span-full py-20 grayscale opacity-40 flex justify-center"><Loader2 size={32} className="animate-spin text-black" /></div> :
                  ...
                  courses.map(course => (
                    <div key={course.id} className="p-8 bg-white border border-gray-100 rounded-[2.5rem] shadow-sm hover:border-black transition-all group relative overflow-hidden">
                       ...
                       <div className="flex-1 min-w-0 mr-12 relative z-10">
                          <div className="flex flex-wrap items-center gap-3 mb-4">
                             <h4 className="text-sm font-black text-black uppercase italic tracking-tight leading-none truncate max-w-[200px]">{course.title}</h4>
                             {course.domain && course.domain !== 'General' && (
                               <span className="text-[8px] font-black tracking-widest uppercase px-2 py-1 rounded bg-black text-white">{course.domain}</span>
                             )}
                          </div>
                       ...
                       <button onClick={() => handleDelete(course.id, course.title)}
                         className="absolute right-8 top-1/2 -translate-y-1/2 p-4 rounded-2xl bg-gray-50 border border-gray-100 hover:border-black hover:bg-black hover:text-white text-gray-300 transition-all z-10">
                         <Trash2 size={16} />
                       </button>
                    </div>
                  ))
      ...
      {activeTab === 'users' && (
        <div className="space-y-8">
           <div className="space-y-1">
              <h3 className="text-3xl font-black italic tracking-tighter text-black uppercase">Universal Ledger</h3>
              <p className="text-[9px] font-black tracking-[0.3em] text-gray-400 uppercase">IDENTIFIED AGENTS: {users.length}</p>
           </div>
           
           <div className="space-y-4">
              {usersLoading ? <div className="py-20 flex justify-center grayscale opacity-40"><Loader2 size={32} className="animate-spin text-black" /></div> :
                  ...
                  <div key={u.id} className="bg-white border border-gray-100 rounded-[2rem] overflow-hidden transition-all hover:bg-gray-50/50">
                    <button onClick={() => fetchUserPerformance(u.id)}
                      ...
                      <div className="flex items-center gap-6 min-w-0 flex-1">
                        <div className="w-14 h-14 rounded-2xl bg-black flex items-center justify-center text-white text-lg font-black shrink-0 shadow-lg shadow-black/5">
                          {(u.name || u.email || '??').substring(0, 2).toUpperCase()}
                        </div>
                        <div className="min-w-0 flex-1 space-y-1">
                          <p className="text-sm font-black text-black uppercase italic tracking-tight truncate">{u.name || 'ANONYMOUS'}</p>
                      ...
                      <div className="flex items-center gap-10 shrink-0 ml-8">
                        <div className="hidden lg:flex gap-10 text-right">
                          <div className="space-y-1">
                             <p className="text-xl font-black italic leading-none text-black">{u.stats?.coursesEnrolled || 0}</p>
                             <p className="text-[8px] text-gray-300 font-black uppercase tracking-widest italic">NODES</p>
                          </div>
                          <div className="space-y-1">
                             <p className="text-xl font-black italic leading-none text-black">{u.stats?.avgQuizScore || 0}%</p>
                             <p className="text-[8px] text-gray-300 font-black uppercase tracking-widest italic">ACCURACY</p>
                          </div>
                          <div className="space-y-1">
                             <p className="text-xl font-black italic leading-none text-black">{u.stats?.highestLevel || 3}★</p>
                    ...
                    {expandedUser === u.id && (
                      <div className="p-10 border-t border-gray-50 bg-white animate-in slide-in-from-top-4 duration-500">
                        {perfLoading[u.id] ? <div className="text-center py-10 grayscale opacity-40"><Loader2 size={24} className="animate-spin text-black" /></div> : 
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            ...
                            <div className="space-y-6">
                               <div className="flex items-center gap-3">
                                  ...
                                  <h4 className="text-[10px] font-black uppercase tracking-widest text-black">Active Node Synchronization</h4>
                               </div>
                               <div className="space-y-3">
                                  {userPerformance[u.id].progress?.length ? userPerformance[u.id].progress.map((p, i) => (
                                    <div key={i} className="flex justify-between items-center p-5 bg-gray-50 border border-gray-100 rounded-2xl">
                                      <span className="text-xs font-black text-black uppercase italic truncate mr-4">{p.course?.title || 'DOMAIN'}</span>
                                      <span className="text-[10px] font-black text-white bg-black px-3 py-1 rounded-lg shrink-0">{p.level || 3}★</span>
                                    </div>
                            ...
                            <div className="space-y-6">
                               <div className="flex items-center gap-3">
                                  <div className="w-1 h-4 bg-black" />
                                  <h4 className="text-[10px] font-black uppercase tracking-widest text-black">Historical Calibration Data</h4>
                               </div>
